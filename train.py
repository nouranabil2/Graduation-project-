...
from core.YOLOv4 import YOLOv4 ,decode_output,compute_loss
...
if __name__=="__main__":
    ...
    STRIDES ,ANCHORS,XYSCALE,NUM_CLASS = utils.load_config()
    IOU_LOSS_THRESH = cfg.YOLO.IOU_LOSS_THRESH
    freeze_layers = utils.load_freeze_layer()
    feature_map = YOLOv4(input_layer,NUM_CLASS)
    bounding_boxes =[]
    for i ,feature in enumerate(feature_map):
        if i == 0:
            bounding_box = decode_output(feature,cfg.TRAIN.INPUT_SIZE//8,NUM_CLASS,STRIDES,ANCHORS,i,XYSCALE)
        elif i == 1:
